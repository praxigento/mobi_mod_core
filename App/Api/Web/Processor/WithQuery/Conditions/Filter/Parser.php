...
class Parser
{
     ...
     *
     * @param \Praxigento\Core\App\Api\Web\Request\Conditions\Filter $filter
    ...
    public function parse($filter, $aliases)
    {
        ...
        if (is_array($clause)) $clause = new \Praxigento\Core\App\Api\Web\Request\Conditions\Filter\Clause($clause);
        $group = $filter->getGroup();
        if (is_array($group)) $group = new \Praxigento\Core\App\Api\Web\Request\Conditions\Filter\Group($group);
        if ($clause) {
            ...
            $parts = [];
            /** @var \Praxigento\Core\App\Api\Web\Request\Conditions\Filter $entry */
            foreach ($entries as $entry) {
                /* place data into 'filter' container */
                if (is_array($entry)) $entry = new \Praxigento\Core\App\Api\Web\Request\Conditions\Filter($entry);
