...
class Parser
{
    ...
    public function parse($filter, $aliases)
    {
        ...
        if (is_array($clause)) $clause = new \Praxigento\Core\Api\App\Web\Request\Conditions\Filter\Clause($clause);
        $group = $filter->getGroup();
        if (is_array($group)) $group = new \Praxigento\Core\Api\App\Web\Request\Conditions\Filter\Group($group);
        if ($clause) {
            ...
            $parts = [];
            /** @var \Praxigento\Core\Api\App\Web\Request\Conditions\Filter $entry */
            foreach ($entries as $entry) {
                /* place data into 'filter' container */
                if (is_array($entry)) $entry = new \Praxigento\Core\Api\App\Web\Request\Conditions\Filter($entry);
