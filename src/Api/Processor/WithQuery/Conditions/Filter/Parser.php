...
namespace Praxigento\Core\Api\Processor\WithQuery\Conditions\Filter;
...
class Parser
{
     ...
     *
     * @param \Praxigento\Core\Api\Request\Part\Conditions\Filter $filter
     * @param \Praxigento\Core\Api\Processor\WithQuery\Alias[] $aliases map of the aliases to [table, field]
     * @return string
     */
    public function parse($filter, $aliases)
    {
        ...
        if (is_array($clause)) $clause = new \Praxigento\Core\Api\Request\Part\Conditions\Filter\Clause($clause);
        $group = $filter->getGroup();
        if (is_array($group)) $group = new \Praxigento\Core\Api\Request\Part\Conditions\Filter\Group($group);
        if ($clause) {
            ...
            $parts = [];
            /** @var \Praxigento\Core\Api\Request\Part\Conditions\Filter $entry */
            foreach ($entries as $entry) {
                /* place data into 'filter' container */
                if (is_array($entry)) $entry = new \Praxigento\Core\Api\Request\Part\Conditions\Filter($entry);
